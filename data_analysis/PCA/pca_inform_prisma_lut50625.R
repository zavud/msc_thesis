...
lut_files = list.files(path = lut_path, pattern = "50625", full.names = T)
...
lut[, 1:231] = lut[, 1:231] + rnorm(n = ncol(lut[, 1:231]) * nrow(lut[, 1:231]), mean = 0, sd = sds * .00001)
...
training = training_val[1:30500,]
validation = training_val[-c(1:30500), ]
...
apply(prisma_df, 2, mean)[1:10]
apply(lut[, 1:231], 2, mean)[1:10]
