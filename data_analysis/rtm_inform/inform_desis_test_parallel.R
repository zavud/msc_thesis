...
fwhm_path = "C:\\Users\\zavud\\Desktop\\msc_thesis\\data_analysis\\rtm_inform\\sensor_metadata\\fwhm_desis.txt"
...
df_desis_sensor = data.frame(center = wl, fwhm = fwhm)
...
LUT = foreach(i = 1:nrow(combinations), .packages = c("ccrtm", "hsdar"), .combine = rbind, .inorder = FALSE) %dopar% {
        ...
        m = spectralResampling(m, sensor = df_desis_sensor)
...
e_foreach_loop - s_foreach_loop # Time difference of 1.11567 hours
