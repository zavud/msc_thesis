...
# set wd
wd = "C:\\Users\\zavud\\Desktop\\msc_thesis\\data_analysis\\inform_prisma"
setwd(wd); rm(wd)
...
Cab = seq(40, 80, length.out = 15)
Cw = seq(0.0035, 0.035, length.out = 13)
Cm = seq(0.008, 0.03, length.out = 13)

# variables for 4SAIL
LAI = seq(0.001, 9, length.out = 14)
...
lut = foreach(i = 1:nrow(combinations), .packages = c("ccrtm", "hsdar"), .combine = rbind, .inorder = FALSE) %dopar% {
        ...
        c(m, combinations[i, "Cab"], combinations[i, "Cw"], combinations[i, "Cm"], combinations[i, "LAI"])
...
dimnames(lut) = NULL
s_writing = Sys.time()
readr::write_csv(as_tibble(lut), file = "C:\\Users\\zavud\\Desktop\\msc_thesis\\data_analysis\\inform_prisma\\lut_database\\lut35490_inform5_alpha.txt")
e_writing = Sys.time()
e_writing - s_writing