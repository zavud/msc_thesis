...
Table (\@ref(tab:tsvaried)) show the 6 parameters that were varied, their units, minimum and maximum values. Each parameter was varied 15 times, meaning 15 different spectra were simulated for each variable.
...
Table (\@ref(tab:tsfixed)) show the determined default values for each INFORM parameter that were kept during the sensitivity simulation while one of the parameter was varied (Table (\@ref(tab:tsvaried))).
...
*Solar zenith angle* and *Sun-sensor azimuth angle* were calculated based on the PRISMA image acquisition parameters (date, lat/long etc.) using the website https://www.esrl.noaa.gov/gmd/grad/solcalc/azel.html.

RTM models PROSPECT5, 4SAIL and FLIM were coupled (INFORM) in order to simulate canopy reflectance. Simulations were carried out using the *ccrtm* package [@ccrtm] in *R* [@r]. The default soil spectra provided by the the *ccrtm* package [@ccrtm] was used for the simulations.
