...
First, the simulated canopy reflectance data was normalized and standardized using the Equation \@ref(eq:norm):
...
PCA produces new variables called Principal Components and each Principal Component (PC) contains certain amount of variation available in the original data. Typically first PC contains the most variation, the second PC contains the second most variation and so on [@bro2014principal]. In this research, PCA was performed on the normalized and scaled simulated data using the packages *recipes* [@recipes] in *tidymodels* [@tidymodels]. Cumulative sum of the variations the PCs contain was calculated in order to assess the proportion of the variation that can be explained with fewer variables than the original data (LUT).
