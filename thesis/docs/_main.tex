...
%
\def\crest{{\includegraphics[scale=0.6]{templates/uni-logo.png}}}
...
\renewcommand{\gapbeforecrest}{12mm}
\renewcommand{\gapaftercrest}{12mm}
...
Radiative transfer models take advantage of physical laws that determine ``causal and effect'' relationships. There have been various radiative transfer models developed (leaf, canopy etc.) during the last decades. RTMs can be used in forward and inverse modes. In the forward mode, biophysical leaf and canopy input variables are used to simulate spectral canopy reflectance. In the inverse mode, measured or simulated reflectance spectra is used as an input in order to retrieve plant biophysical parameters. Radiative transfer models can be used to retrieve forest canopy biophysical and biochemical variables from remote sensing satellite data \citep{verrelst2019quantifying}.
...
Leaf carotenoid content & Car & $\frac{\mu g}{cm^2}$\\
...
The SAIL model can be considered to be a relatively simple 1D model and takes only a few variables as its inputs. SAIL requires leaf reflectance and transmittance as well as background reflectance. Leaf transmittance and reflectance can be simulated using the PROSPECT model and directly fed into the SAIL model \citep{berger2018evaluation}. Table \ref{tab:4sail} shows the input variables that the 4SAIL model requires.
...
The parameters observer zenith angle, sun zenith angle and sun-sensor azimuth angle (Table \ref{tab:4sail}) are usually to be provided to the SAIL model explicitly. The parameter \emph{Hotspot} refers to the ratio between leaf size and canopy height \citep{berger2018evaluation}. It is important to note that SAIL is mainly designed to work better with relatively homogeneous forests and canopies in general \citep{jay2017retrieving}.
...
Leaf carotenoid content & $C_{ar}$ & $\frac{\mu g}{cm^2}$ & 8 & 8 & $-$\\
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/stats} \caption{Mean and mean $\pm$ standard deviation in the a) LUT and b) PRISMA image (all pixels within the study area)}\label{fig:statplots}
\end{figure}
